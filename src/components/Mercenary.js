...
class Mercenary extends Component {
  ...
  onClickSetCode = _uniqueCode => {
    const mercenarySelected =
      this.props.dataset.mercenarySelected === _uniqueCode ? null : _uniqueCode;
  ...
  render() {
    ...
    const opacity = _.isUndefined(formation.find(f => f.uniqueCode === _uniqueCode)) ? 1 : 0.2;

    return (
      <div
        className={`mercenary ${nops[0] ? 'show' : ''} ${nops[1] ? 'bold' : ''} ${
          mercenarySelected === _uniqueCode && opacity === 1 ? 'selected' : ''
        }`}
