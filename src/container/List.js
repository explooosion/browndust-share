...
class List extends Component {
  ...
  getListBySearch = (search = '', characters) => {
    ...
    return this.props.charactersGlobal.filter(c => {
      ...
      switch (this.props.settings.locale) {
        case 'US':
          match = _charName_ENG.indexOf(search) > -1;
          break;
        case 'ES':
          match = _charName_SPA.indexOf(search) > -1;
          break;
        case 'DE':
          match = _charName_GER.indexOf(search) > -1;
          break;
        case 'TW':
          match = _charName_TW.indexOf(search) > -1 || sify(_charName_TW).indexOf(search) > -1;
          break;
        case 'CN':
          match = _charName_TW.indexOf(search) > -1 || sify(_charName_TW).indexOf(search) > -1;
          break;
        case 'JP':
          match = _charName_JAP.indexOf(search) > -1;
          break;
        case 'KR':
          match = _charName.indexOf(search) > -1;
          break;
        case 'TH':
          match = _charName_TH.indexOf(search) > -1;
          break;
        default:
          match = true;
      }
  ...
  renderCharacters(characters) {
    return this.getListBySearch(this.state.search, characters)
      .filter(character => {
        const { _type, _growType, _star } = character;
        return (
          Number(_type) === this.state.type &&
          Number(_growType) >= 0 &&
          ((this.state.stars[0].checked && Number(_star) === 6) ||
            (this.state.stars[1].checked && Number(_star) === 5) ||
      ...
      .map((c, index) => {
        console.log(c);
        return (
          <Mercenary key={`mercenary-${index}`} params={c} nameOptions={this.state.nameOptions} />
        );
      });
  }
  ...
  render() {
    ...
    return (
      ...
      >
        <div className="types">{this.renderTypes()}</div>
        ...
        <div className="content">
          {characters.length === 0 ? (
            <HashLoader css={override} color={'#5ac0de'} size={100} />
          ) : (
